        ...
        {media.map((item) => (
          <div key={item.id} className="relative group">
            <div className="aspect-square bg-gray-200 rounded-lg overflow-hidden cursor-pointer"
              ...
              {/* Overlay */}
              <div className="absolute inset-0 bg-[rgba(0,0,0,0)] bg-opacity-0 group-hover:bg-opacity-20 transition-all flex items-center justify-center">
                {item.type === 'video' && (
      ...
      <Dialog open={!!selectedMedia} onOpenChange={() => setSelectedMedia(null)}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              View the selected media file in full size.
            </DialogDescription>
          </DialogHeader>
          {selectedMedia && (
