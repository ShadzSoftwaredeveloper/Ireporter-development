...
export const UserProfile: React.FC = () => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Store file and preview; actual upload happens when user clicks Save
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      let profilePictureValue = editProfilePicture;

      // If user selected a new file, upload it via multipart to /api/upload
      if (selectedFile) {
        const formData = new FormData();
        // upload middleware expects field name 'media' and supports multiple files
